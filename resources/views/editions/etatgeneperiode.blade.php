    ...
    <div class="content-wrapper">
        <section class="content-header">
            <div class="container-fluid">
                <div class="row">
                    <div class="col-sm-6">
                        <h1 class="pb-3">ETAT DE GENERAL D'UNE PERIODE DES REGLEMENTS CONTROLE</h1>
                    </div>
                    <div class="col-sm-6">
                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="card card-secondary">
                            <div class="card-body">
                                <form method="post" id="form_bc" action="{{route('edition.postEtatGenePeriode')}}">
                                    @csrf
                                                ...
                                                <table class="table table-bordered table-striped table-sm mt-2"  style="font-size: 12px">
                                                    ...
                                                    @foreach(session('resultat')['reglements'] as $key=>$item)
                                                                <tr>
                                                                    ...
                                                                    <td class="text-center">{{date_format(date_create($item->created_at),'d/m/Y')}}</td>
                                                                    <td class="text-center">{{ $item->reference }}</td>
                                                                    <td class="text-center">{{ $item->utilisateur->name}}</td>
                                                                    {{-- <td class="text-center">{{ $item->vente->commandeclient}}</td> --}}
                                                                    <td>
                                                                        @if ($item->document)
                                                                        <a  class="btn btn-success float-md-right text-white btn-sm" href="{{ $item->document?asset('storage/'.$item->document):'' }}" target="_blank"><i class="fa-solid fa-file-pdf"></i></a>
                                                                        @endif
                                                                    </td>
...
@section('script')
    <script>
        $(document).ready(function() {
            $("tr").each(function() {
                var $cells = $(this).find("td:nth-child(2)"); // Sélectionner la deuxième colonne

                $cells.each(function() {
                var $cell = $(this);
                var $prevCell = $cell.prevAll();

                if ($prevCell.length > 0 && $prevCell.text() === $cell.text()) {
                    $$prevCell.attr("rowspan", 1);
                    $cell.remove();
                } else {
                    $cell.attr("rowspan", 1);
                }
